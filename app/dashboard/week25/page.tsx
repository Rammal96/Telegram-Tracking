...
import StartWeek25Button from './start-button'
...
async function getWeek26StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
      .eq('key', 'week26_start_time')
      .single()
...
async function getClicks() {
  try {
    ...
    const week26StartTime = await getWeek26StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 25 start time until Week 26 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 26+ clicks once Week 26 has started
      if (week26StartTime) {
        query = query.lt('timestamp', week26StartTime)
      }
...
export default async function Week25Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 25 tracking has not started yet.</p>
          <StartWeek25Button />
        </div>
      )}
      <DashboardClient 

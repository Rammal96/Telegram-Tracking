...
import StartWeek10Button from './start-button'
...
async function getWeek11StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
...
async function getClicks() {
  try {
    ...
    const week11StartTime = await getWeek11StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 10 start time until Week 11 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 11 clicks if Week 11 has started
      if (week11StartTime) {
        query = query.lt('timestamp', week11StartTime)
      }
...
export default async function Week10Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 10 tracking has not started yet.</p>
          <StartWeek10Button />
        </div>
      )}
      <DashboardClient 

...
import StartWeek14Button from './start-button'
...
async function getWeek15StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
      .eq('key', 'week15_start_time')
      .single()
...
async function getClicks() {
  try {
    ...
    const week15StartTime = await getWeek15StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 14 start time until Week 15 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 15 clicks if Week 15 has started
      if (week15StartTime) {
        query = query.lt('timestamp', week15StartTime)
      }
...
export default async function Week14Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 14 tracking has not started yet.</p>
          <StartWeek14Button />
        </div>
      )}
      <DashboardClient 

...
import StartWeek7Button from './start-button'
...
async function getWeek8StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
...
async function getClicks() {
  try {
    ...
    const week8StartTime = await getWeek8StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 7 start time until Week 8 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 8 clicks if Week 8 has started
      if (week8StartTime) {
        query = query.lt('timestamp', week8StartTime)
      }
...
export default async function Week7Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 7 tracking has not started yet.</p>
          <StartWeek7Button />
        </div>
      )}
      <DashboardClient 

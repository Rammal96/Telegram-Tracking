...
import StartWeek24Button from './start-button'
...
async function getWeek25StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
      .eq('key', 'week25_start_time')
      .single()
...
async function getClicks() {
  try {
    ...
    const week25StartTime = await getWeek25StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 24 start time until Week 25 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 25+ clicks once Week 25 has started
      if (week25StartTime) {
        query = query.lt('timestamp', week25StartTime)
      }
...
export default async function Week24Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 24 tracking has not started yet.</p>
          <StartWeek24Button />
        </div>
      )}
      <DashboardClient 

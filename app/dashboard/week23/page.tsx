...
import StartWeek23Button from './start-button'
...
async function getWeek24StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
      .eq('key', 'week24_start_time')
      .single()
...
async function getClicks() {
  try {
    ...
    const week24StartTime = await getWeek24StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 23 start time until Week 24 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 24+ clicks once Week 24 has started
      if (week24StartTime) {
        query = query.lt('timestamp', week24StartTime)
      }
...
export default async function Week23Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 23 tracking has not started yet.</p>
          <StartWeek23Button />
        </div>
      )}
      <DashboardClient 

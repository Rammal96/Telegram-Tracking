...
import StartWeek20Button from './start-button'
...
async function getWeek21StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
...
async function getClicks() {
  try {
    ...
    const week21StartTime = await getWeek21StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 20 start time until Week 21 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 21+ clicks once Week 21 has started
      if (week21StartTime) {
        query = query.lt('timestamp', week21StartTime)
      }
...
export default async function Week20Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 20 tracking has not started yet.</p>
          <StartWeek20Button />
        </div>
      )}
      <DashboardClient 

...
import StartWeek15Button from './start-button'
...
async function getWeek16StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
      .eq('key', 'week16_start_time')
      .single()
...
async function getClicks() {
  try {
    ...
    const week16StartTime = await getWeek16StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 15 start time until Week 16 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 16 clicks if Week 16 has started
      if (week16StartTime) {
        query = query.lt('timestamp', week16StartTime)
      }
...
export default async function Week15Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 15 tracking has not started yet.</p>
          <StartWeek15Button />
        </div>
      )}
      <DashboardClient 

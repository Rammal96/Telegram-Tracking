...
import StartWeek16Button from './start-button'
...
async function getWeek17StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
      .eq('key', 'week17_start_time')
      .single()
...
async function getClicks() {
  try {
    ...
    const week17StartTime = await getWeek17StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 16 start time until Week 17 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 17 clicks if Week 17 has started
      if (week17StartTime) {
        query = query.lt('timestamp', week17StartTime)
      }
...
export default async function Week16Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 16 tracking has not started yet.</p>
          <StartWeek16Button />
        </div>
      )}
      <DashboardClient 

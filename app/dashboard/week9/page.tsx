...
import StartWeek9Button from './start-button'
...
async function getWeek10StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
...
async function getClicks() {
  try {
    ...
    const week10StartTime = await getWeek10StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 9 start time until Week 10 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 10 clicks if Week 10 has started
      if (week10StartTime) {
        query = query.lt('timestamp', week10StartTime)
      }
...
export default async function Week9Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 9 tracking has not started yet.</p>
          <StartWeek9Button />
        </div>
      )}
      <DashboardClient 

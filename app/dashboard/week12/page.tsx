...
import StartWeek12Button from './start-button'
...
async function getWeek13StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
...
async function getClicks() {
  try {
    ...
    const week13StartTime = await getWeek13StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 12 start time until Week 13 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 13 clicks if Week 13 has started
      if (week13StartTime) {
        query = query.lt('timestamp', week13StartTime)
      }
...
export default async function Week12Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 12 tracking has not started yet.</p>
          <StartWeek12Button />
        </div>
      )}
      <DashboardClient 

...
import StartWeek17Button from './start-button'
...
async function getWeek18StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
...
async function getClicks() {
  try {
    ...
    const week18StartTime = await getWeek18StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 17 start time until Week 18 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 18 clicks if Week 18 has started
      if (week18StartTime) {
        query = query.lt('timestamp', week18StartTime)
      }
...
export default async function Week17Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 17 tracking has not started yet.</p>
          <StartWeek17Button />
        </div>
      )}
      <DashboardClient 

...
import StartWeek13Button from './start-button'
...
async function getWeek14StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
...
async function getClicks() {
  try {
    ...
    const week14StartTime = await getWeek14StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 13 start time until Week 14 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 14 clicks if Week 14 has started
      if (week14StartTime) {
        query = query.lt('timestamp', week14StartTime)
      }
...
export default async function Week13Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 13 tracking has not started yet.</p>
          <StartWeek13Button />
        </div>
      )}
      <DashboardClient 

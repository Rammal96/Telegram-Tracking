...
import StartWeek21Button from './start-button'
...
async function getWeek22StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
      .eq('key', 'week22_start_time')
      .single()
...
async function getClicks() {
  try {
    ...
    const week22StartTime = await getWeek22StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 21 start time until Week 22 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 22+ clicks once Week 22 has started
      if (week22StartTime) {
        query = query.lt('timestamp', week22StartTime)
      }
...
export default async function Week21Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 21 tracking has not started yet.</p>
          <StartWeek21Button />
        </div>
      )}
      <DashboardClient 

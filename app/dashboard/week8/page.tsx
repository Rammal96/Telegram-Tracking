...
import StartWeek8Button from './start-button'
...
async function getWeek9StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
      .eq('key', 'week9_start_time')
      .single()
...
async function getClicks() {
  try {
    ...
    const week9StartTime = await getWeek9StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 8 start time until Week 9 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 9 clicks if Week 9 has started
      if (week9StartTime) {
        query = query.lt('timestamp', week9StartTime)
      }
...
export default async function Week8Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 8 tracking has not started yet.</p>
          <StartWeek8Button />
        </div>
      )}
      <DashboardClient 

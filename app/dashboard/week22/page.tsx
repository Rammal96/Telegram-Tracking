...
import StartWeek22Button from './start-button'
...
async function getWeek23StartTime(): Promise<string | null> {
  try {
    const { data, error } = await supabase
      .from('settings')
      .select('value')
...
async function getClicks() {
  try {
    ...
    const week23StartTime = await getWeek23StartTime()
    ...
    while (hasMore) {
      ...
      // Filter clicks from Week 22 start time until Week 23 starts
      if (startTime) {
        query = query.gte('timestamp', startTime)
      }

      // Exclude Week 23+ clicks once Week 23 has started
      if (week23StartTime) {
        query = query.lt('timestamp', week23StartTime)
      }
...
export default async function Week22Dashboard() {
    ...
    <div>
      {!startTime && (
        <div className="mb-6 bg-yellow-900/50 border border-yellow-400/30 p-4 rounded-lg">
          <p className="text-yellow-400 mb-3">Week 22 tracking has not started yet.</p>
          <StartWeek22Button />
        </div>
      )}
      <DashboardClient 

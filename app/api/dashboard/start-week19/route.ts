...
export async function POST(request: NextRequest) {
  try {
    ...
    // Set the start time for Week 19 tracking (current timestamp)
    const startTime = new Date().toISOString()

    const { data, error } = await supabase
      .from('settings')
      .upsert({
        key: 'week19_start_time',
        value: startTime,
    ...
    if (error) {
      console.error('Error setting Week 19 start time:', error)
    ...
    return NextResponse.json({ 
      success: true, 
      startTime,
      message: 'Week 19 tracking started'
    })
  } catch (error) {
    console.error('Exception setting Week 19 start time:', error)

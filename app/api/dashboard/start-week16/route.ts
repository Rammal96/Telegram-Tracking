...
export async function POST(request: NextRequest) {
  try {
    ...
    // Set the start time for Week 16 tracking (current timestamp)
    const startTime = new Date().toISOString()

    const { data, error } = await supabase
      .from('settings')
      .upsert({
        key: 'week16_start_time',
        value: startTime,
    ...
    if (error) {
      console.error('Error setting Week 16 start time:', error)
    ...
    return NextResponse.json({ 
      success: true, 
      startTime,
      message: 'Week 16 tracking started'
    })
  } catch (error) {
    console.error('Exception setting Week 16 start time:', error)

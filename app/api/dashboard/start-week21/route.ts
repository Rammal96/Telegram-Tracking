...
export async function POST(request: NextRequest) {
  try {
    ...
    // Set the start time for Week 21 tracking (current timestamp)
    const startTime = new Date().toISOString()

    const { data, error } = await supabase
      .from('settings')
      .upsert({
        key: 'week21_start_time',
        value: startTime,
    ...
    if (error) {
      console.error('Error setting Week 21 start time:', error)
    ...
    return NextResponse.json({ 
      success: true, 
      startTime,
      message: 'Week 21 tracking started'
    })
  } catch (error) {
    console.error('Exception setting Week 21 start time:', error)
